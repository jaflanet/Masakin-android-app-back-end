...
exports.addAccount = async (req, res) => {
  await cloudinary.uploader.upload(
    ...
    { dpr: "auto", responsive: true, width: "auto", crop: "scale" },
    (error, result) => {
      const query = `INSERT INTO account(username, password, email, name, accPhoneNumber, profilePicture, accountType) VALUES ('${req.body.username}', '${req.body.password}', '${req.body.email}', '${req.body.name}', '${req.body.accPhoneNumber}', '${result.secure_url}', '${req.body.accountType}')`;
      pool.execute(query, function (err, result) {
        if (err) {
          res.send("error");
          throw err;
